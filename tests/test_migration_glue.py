...
# 这里只 characterize server 侧 glue。所有 server helper(get_client/_safe_write_text/vault_git/
# _get_schema_version/_self_evolve_run...)抽出时留 server,migration_routes lazy import。
...
def reset_migration_state():
    ...
    server._migration_log = []
    server._migration_consumers = []
    server._migration_done = False
    server._migration_lock = None
    yield
...
def test_push_event_appends_to_log_and_fanouts_to_consumers():
    ...
    server._migration_consumers.append(q)
    asyncio.run(server.push_migration_event({"kind": "file_done", "name": "x"}))
    assert server._migration_log[-1]["kind"] == "file_done"
    assert server._migration_done is False              # 非终态不置 done
...
def test_push_terminal_kind_marks_done():
    for kind in ("migration_done", "migration_skipped"):
        server._migration_done = False
        asyncio.run(server.push_migration_event({"kind": kind}))
        assert server._migration_done is True


def test_stream_replays_snapshot_then_closes_on_terminal(client):
    # 已 done + log 有终态事件 → 新连接 replay 快照后立即收尾(不挂)
    server._migration_log = [{"kind": "file_done", "name": "a"},
                             {"kind": "migration_done", "success": True}]
    server._migration_done = True
...
def test_create_migration_llm_none_when_no_client(monkeypatch):
    monkeypatch.setattr(server, "get_client", lambda *a, **k: None)
    assert server._create_migration_llm() is None
...
def test_startup_skips_run_migration_when_llm_none(monkeypatch):
    ...
    asyncio.run(server._startup_v0125_md_migration())
...
def test_startup_closes_banner_on_engine_exception(monkeypatch):
    ...
    monkeypatch.setattr(server, "_create_migration_llm", lambda: object())
...
def test_reference_bootstrap_copies_missing(iso_vault):
    res = server._ensure_vault_reference_files()
...
def test_schema_migration_noop_when_no_bundle_dir(monkeypatch, tmp_path):
    ...
    asyncio.run(server._run_schema_migration_if_needed())
